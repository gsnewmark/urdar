...
(defn delete-tag
  ...
  [tag-node]
  (nrl/purge-all tag-node)
  (nn/delete tag-node))

(defn any-bookmarks-tagged?
  [user-node tag-node]
  (not (empty? (get-bookmarks-for-tag user-node tag-node 0 1))))
...
(defn untag-bookmark-node
  "Adds a given link to a given tag. Returns nil if bookmark already has
  given tag."
  [tag-node link-node]
  (if-let [rel (get-tagged tag-node link-node)]
    (nrl/delete (:id rel))))
